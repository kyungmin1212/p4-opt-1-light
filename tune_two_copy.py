...
def search_hyperparam(trial: optuna.trial.Trial) -> Dict[str, Any]:
    ...
    epochs = trial.suggest_int("epochs", low=20, high=20, step=10)
    img_size = trial.suggest_categorical("img_size", [112])
    n_select = trial.suggest_int("n_select", low=0, high=0, step=1)
...
def search_model(trial: optuna.trial.Trial) -> List[Any]:
    ...
    if m2 == "Conv":
        ...
        m2_sq=trial.suggest_int("m2/squeeze",low=8,high=64,step=8)
        m2_e1=m2_sq*4
        m2_e3=m2_sq*4
        m2_args=[m2_sq,m2_e1,m2_e3]
    if not m2 == "Pass":
        if m2_stride == 2:
            n_stride += 1
    ...
    if m3 == "Conv":
        ...
        m3_sq=trial.suggest_int("m3/squeeze",low=8,high=64,step=8)
        m3_e1=m3_sq*4
        m3_e3=m3_sq*4
        m3_args=[m3_sq,m3_e1,m3_e3]
    if not m3 == "Pass":
        if m3_stride == 2:
            n_stride += 1
            if n_stride>=MAX_NUM_STRIDE:
    ...
    if m4 == "Conv":
        ...
        m4_sq=trial.suggest_int("m4/squeeze",low=8,high=64,step=8)
        m4_e1=m4_sq*4
        m4_e3=m4_sq*4
        m4_args=[m4_sq,m4_e1,m4_e3]
    if not m4 == "Pass":
        if m4_stride == 2:
            n_stride += 1
            if n_stride>=MAX_NUM_STRIDE:
    ...
    if m5 == "Conv":
        ...
        m5_sq=trial.suggest_int("m5/squeeze",low=8,high=64,step=8)
        m5_e1=m5_sq*4
        m5_e3=m5_sq*4
        m5_args=[m5_sq,m5_e1,m5_e3]
    if not m5 == "Pass":
        if m5_stride == 2:
            n_stride += 1
            if n_stride>=MAX_NUM_STRIDE:
    ...
    if m6 == "Conv":
        ...
        m6_sq=trial.suggest_int("m6/squeeze",low=8,high=64,step=8)
        m6_e1=m6_sq*4
        m6_e3=m6_sq*4
        m6_args=[m6_sq,m6_e1,m6_e3]
    if not m6 == "Pass":
        if m6_stride == 2:
            n_stride += 1
            if n_stride>=MAX_NUM_STRIDE:
    ...
    if m7 == "Conv":
        ...
        m7_sq=trial.suggest_int("m7/squeeze",low=8,high=64,step=8)
        m7_e1=m7_sq*4
        m7_e3=m7_sq*4
        m7_args=[m7_sq,m7_e1,m7_e3]
    if not m7 == "Pass":
        if m7_stride == 2:
            n_stride += 1
            if n_stride>=MAX_NUM_STRIDE:
...
def objective(trial: optuna.trial.Trial, device) -> Tuple[float, int, float]:
    ...
    """
    wandb.init(project='3_300_20_scoresearch',reinit=True)
    ...
    score = macs/13863553+(1-test_f1/0.8342)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--study-name", default="3_300_20_scoresearch", type=str, help="Optuna study name.")
