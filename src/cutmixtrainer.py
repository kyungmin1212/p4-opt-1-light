...
class TorchTrainer:
    ...
    def generate_cutmix_image(self,image_batch, image_batch_labels,num_classes, beta=1):
        ...
        target_a = self.one_hot_encoding(image_batch_labels,num_classes)
        target_b = self.one_hot_encoding(image_batch_labels[rand_index],num_classes)
        bbx1, bby1, bbx2, bby2 = self.rand_bbox(image_batch[0].shape, lam)
    ...
    def train(
        ...
        for epoch in range(n_epoch):
            ...
            for batch, (data, labels) in pbar:
                data=data.permute(0,2,3,1).numpy()  # cutmix
                data, labels=self.generate_cutmix_image(data, labels,num_classes)  # cutmix
                data=torch.from_numpy(data)  # cutmix
                data=data.permute(0,3,1,2)  # cutmix
                ...
                labels=torch.argmax(labels,dim=-1)  # cutmix
                total += labels.size(0)
    ...
    def test(
        ...
        model.eval()
        for batch, (data, labels) in pbar:
            labels=self.one_hot_encoding(labels,num_classes)  # cutmix
            data, labels = data.to(self.device), labels.to(self.device)
            if self.scaler:
                with torch.cuda.amp.autocast():
                    outputs = model(data)
